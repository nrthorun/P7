...
* Conversão: Número único de usuários a matricularem no free trial dividido pelo número único de cookies que clicaram no botão _'start free trial'_. No grupo de experimento deve-se aparecer a janela com a pergunta a respeito das horas semanais disponíveis e no grupo controle, não. Por isso, é uma boa métrica de avaliação

* Retenção: Número de usuários que permaneceram matriculados por mais de 14 dias (e, portanto, fazendo pelo menos 1 pagamento) sobre o número de usuários matriculados. Essas etapas acontecem após o experimento, os números nos grupos podem ser diferentes. Métrica de avaliação.
...
Métricas não utilizadas: Número de usuários únicos. Como depende do experimento na página de _'start free trial'_, não pode ser utilizado como métrica invariante. Poderia ser utilizada como métrica de avaliação, porém não traz informações muito pertinentes e é redundante às outras métricas de avaliação.
...
A unidade de análise é uma pessoa que clica no botão e a unidade de desvio é um cookie. As unidades não são iguais, porém são próximas o bastante para sugerir que as estimativas empírica e analítica serão bem aproximadas.
...
A unidade de análise é uma pessoa que se matriculou e a unidade de desvio é um user-id. É bem provável que cada pessoa só se matricule uma vez. Considerando isso, as estimativas devem ser aproximadas.
...
Aqui as unidades de análise e de desvio são iguais. Sendo assim, devemos ter a estimativa analítica mais precisa.
...
* Métricas de Avaliação: A métrica de avaliação seria Retenção. Um aumento estatisticamente e praticamente significante significaria sucesso do experimento.
